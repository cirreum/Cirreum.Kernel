...
/// <summary>
/// Guards the <c>custom*</c> → native claim aliasing that both runtimes depend on.
/// </summary>
/// <remarks>
/// The two invariants worth breaking a build over are <b>additive</b> (a native claim is never
/// removed, and precedence is never guessed) and <b>idempotent</b> (the step runs at principal
/// construction on the client and again server-side, so a second pass must be a no-op). The rest
/// pins the wire contract: arrays split into individual claims, the two named claims follow the
/// identity's configured claim types, blank and non-string entries are dropped, and provenance
/// survives the alias.
/// </remarks>
public class CustomClaimCanonicalizerTests {
	...
	public void A_native_claim_is_never_removed_and_the_minted_value_is_unioned_in() {
		...
		// Resolving a conflict is the application's declaration, not this step's.
	...
	public void CustomRoles_aliases_to_the_identitys_configured_role_claim_type() {
		// A provider may configure a non-default role claim type; customRoles must follow it so
		// IsInRole resolves.
	...
	public void A_role_claim_type_configured_as_customRoles_itself_splits_once_and_stays_stable() {
		// An application may point its role claim type directly at the wire name. The alias target
		// is then itself a custom* type; the array still splits so IsInRole works, and a second run
