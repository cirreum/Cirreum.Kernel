...
/// <summary>
/// Aliases provisioned <c>custom*</c> token claims to their native names —
/// <c>customRoles</c> → <c>roles</c>, <c>customName</c> → <c>name</c>, and so on.
/// </summary>
/// <remarks>
/// <para>
/// An application projects claims it owns into the issued token under a <c>custom*</c>
/// namespace, collision-safe against any identity provider's native claim names. This step
/// aliases each one back to the native claim the rest of the framework reads —
/// <see cref="ClaimsPrincipal.IsInRole(string)"/>, the configured name / role claim types,
/// and profile enrichment. It runs on both sides of the wire, at principal construction: in
...
/// provisioned <c>custom*</c> counterpart are present, both survive — this step does not guess
/// which wins. Precedence is the application's declaration, expressed per scheme through
/// <see cref="ClaimAuthority"/>, or resolved by an application's own claims extender.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
public static class CustomClaimCanonicalizer {

	private const string Prefix = "custom";

	// The two native claim names the framework treats specially — the counterpart to the
	// provisioning mint's Roles / Name claims (whose native forms are "roles" / "name"). They
	...
	// unrelated "customer" or "customs" claim is left alone.
	...
	// arrays (the mint's claim type rejects blank values at construction), so null, empty, and
	// non-string entries are dropped rather than minted as claim values. Anything that is not a
	// JSON array passes through as an opaque scalar; an empty scalar produces no alias.
